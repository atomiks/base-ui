...
describe('<Autocomplete.Root />', () => {
  ...
  describe('prop: mode', () => {
    ...
    it('mode="both": autoHighlight inline completes a controlled value while typing', async () => {
      const items = ['apple', 'banana'];
      ...
      // The typed value is the user's own change, so it must not discard the
      // completion `autoHighlight` applied for it.
      await user.type(input, 'a');
      expect(input.value).toBe('apple');
