...
export function useDismiss(
  ...
  React.useEffect(() => {
    ...
    function handlePressEndCapture(event: PointerEvent | MouseEvent) {
      ...
      // the immediate click only.
      if (pressStartedInsideDefaultPrevented) {
        suppressNextOutsideClickRef.current = true;
        // Firefox can dispatch that synthetic click in the same task as `pointerup`;
        // clearing in a microtask preserves suppression for that click only.
        queueMicrotask(() => {
          suppressNextOutsideClickRef.current = false;
        });
        return;
