...
import { isVirtualClick, isVirtualPointerEvent } from '../floating-ui-react/utils/event';
...
export function useOpenInteractionType(open: boolean) {
  ...
  const handleTriggerClick = useStableCallback(
    (event: React.MouseEvent | React.PointerEvent, interactionType: InteractionType) => {
      if (!open) {
        if (
          (event.type === 'pointerdown' &&
            isVirtualPointerEvent(event.nativeEvent as PointerEvent)) ||
          (event.type === 'click' && isVirtualClick(event.nativeEvent as MouseEvent | PointerEvent))
        ) {
